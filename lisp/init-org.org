...
^Hide^             ^Show^           ^Move
^^^^^^------------------------------------------------------
_hs_: sublevels     _sa_: all         _mu_: up
_hb_: body          _se_: entry       _mn_: next visible
_ho_: other         _sc_: children    _mp_: previous visible
_he_: entry         _sb_: branches    _mf_: forward same level
_hl_: leaves        _st_: subtree     _mb_: backward same level
_ht_: subtree
...
"
  ...
  ;; Move
  ("mu" outline-up-heading)                ; Up
  ("mn" outline-next-visible-heading)      ; Next
  ("mp" outline-previous-visible-heading)  ; Previous
  ("mf" outline-forward-same-level)        ; Forward - same level
  ("mb" outline-backward-same-level)       ; Backward - same level
